...
# Профиль бота (BotFather-поля через Bot API, ставятся при старте).
# About: до 120 символов, видно в профиле бота.
BOT_ABOUT = (
    "ИИ-аналитик кабинета Wildberries: продажи, прибыль, остатки, арбитраж, "
    "автоответы на отзывы. Работает 24/7."
)

# Description: до 512 символов, видно на экране «Что умеет этот бот?».
BOT_DESCRIPTION = (
    "Личный ИИ-аналитик продавца Wildberries.\n"
    "\n"
    "📊 Утренний дайджест: заказы, выкупы, остатки\n"
    "💰 Честная прибыль: финотчёты WB, налог, логистика\n"
    "🔥 Алерты падения цен и новые заказы в реальном времени\n"
    "🎯 Арбитраж-сканер связок с расчётом маржи\n"
    "💬 Сам отвечает покупателям на отзывы и вопросы\n"
    "🤖 ИИ-ассистент: проверяет кабинет и чинит настройки, каждое изменение "
    "подтверждается кнопкой\n"
    "\n"
    "Python · aiogram · Ollama Cloud · 277 автотестов"
)
