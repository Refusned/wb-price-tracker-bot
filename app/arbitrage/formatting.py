...
def build_alert_message(
    *,
    ...
    lines = [
        ...
        f"{confidence} СПП категории: {margin.category_spp_pct:.1f}% | "
        f"WB-Кошелёк: {margin.wallet_bonus_pct:.0f}% ({margin.spp_source}, conf={margin.spp_confidence})",
        f"",
        f"━━━━━━━━━━━━━━━━━━━",
        f"💰 Купить (видишь на WB): {_fmt_rub(margin.market_price_rub)}",
        f"   ↳ моя checkout с кошельком: {_fmt_rub(margin.buy_price_rub)}",
        f"💵 Выставить: {_fmt_rub(margin.listed_implied_rub)} (= match competitor)",
        f"   ↳ когорта: P25 {_fmt_rub(market_p25)} | медиана {_fmt_rub(market_median)} | мин {_fmt_rub(market_min)} | n={cohort_size}",
        f"",
        f"📉 Удержания WB (с моей listed):",
