...
_PROPOSE_NAMES = {"propose_purchase", "propose_profit_setting", "propose_feedback_reply"}
_TEMPERATURE = 0.3
_NUM_PREDICT = 1200
...
_SYSTEM_TEMPLATE = (
    ...
    "- Ты ТОЛЬКО ЧИТАЕШЬ данные через инструменты и СОВЕТУЕШЬ. Сам ты НЕ меняешь "
    "цены, не оформляешь закупки, не отвечаешь покупателям, ничего не публикуешь.\n"
    "- Чтобы что-то ИЗМЕНИТЬ — вызови соответствующий инструмент propose_* "
    "(записать закупку, сменить параметр прибыли, ответить покупателю). Это лишь "
