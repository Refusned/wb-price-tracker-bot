...
**Последнее обновление: 2026-05-29 (Day 19) — Этап 1 (color-фильтр) в проде**

---

## ⚡ Quick Resume (для следующей сессии)

**Что сделано (Day 19) — Этап 1 (детерминированная база, БЕЗ LLM):**
- Per-query keyword-фильтр когорты по **структурному полю `colors`** WB (не по заголовку) — чёрные/серые без цвета в названии остаются (часто самые дешёвые связки).
- Деплой + verified на проде: «Станция Миди» cohort 151→46, P25-якорь 11382→12218₽ (деконтаминация).
- Ground-truth метки: таблица `arb_nm_labels` + команды `/arb_mark`, `/arb_keywords`. Миграция m009.
- 85 тестов (добавлен первый характеризационный тест на `scan_once()` — раньше тестов на сканер не было).
- PR #3, ветка `feat/arb-keyword-filter-labels`. `margin.py` НЕ тронут.
- Vision-LLM НЕ понадобился: WB отдаёт цвет структурой в выдаче поиска.
- Контекст: пришли через `/autoplan` (CEO/Eng/DX dual-voice) → план в `~/.claude/plans/wb-bot-llm-ollama-integration.md`.

**Что прямо сейчас работает в проде:**
- Bot: `ai-jobs-vps:/root/WB/` (systemd `wb-bot.service`)
- Scanner каждые 10 мин, threshold `margin≥4%, profit≥300₽, PPRD≥0.5%`
- 2 active queries: "Станция Миди" (**фильтр `include=черн,серый` АКТИВЕН**), "робот пылесос xiaomi" (без фильтра)
- Категория 2791 (Роботы-пылесосы) — без observations → 0 candidates
- ⚠️ `arb_candidates` без ретеншена (~200МБ БД) — follow-up задача

**Завтра делать (NEXT STEPS):**
- Несколько дней `/arb_mark` по ложным алертам → решить, нужен ли Этап 2 (LLM-классификация). Только если метки покажут реальный семантический пробел поверх color-фильтра.
- См. также секцию §8 ниже.
